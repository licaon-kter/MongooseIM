...
set_custom(Host, Module, Level) ->
    ...
    mongoose_helper:successful_rpc(#{node => Node}, mongoose_logs, set_module_level, [Module, Level]).

clear_custom(Host, Module, _Level) ->
    Node = ct:get_config({hosts, Host, node}),
    mongoose_helper:successful_rpc(#{node => Node}, mongoose_logs, unset_module_level, [Module]).
